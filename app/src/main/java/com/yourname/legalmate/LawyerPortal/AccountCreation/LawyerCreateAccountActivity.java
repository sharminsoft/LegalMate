...
public class LawyerCreateAccountActivity extends AppCompatActivity {

    private static final String TAG = "LawyerCreateAccount";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lawyer_create_account);
                        ...
                        if (result.getResultCode() == RESULT_OK && result.getData() != null) {
                            ...
                            try {
                                ...
                                if (account != null && account.getIdToken() != null) {
                                    ...
                                    hideProgressDialog();
                                    Toast.makeText(LawyerCreateAccountActivity.this, "Failed to get account information", Toast.LENGTH_LONG).show();
                                }
                            ...
                            if (result.getResultCode() != RESULT_CANCELED) {
                                Toast.makeText(LawyerCreateAccountActivity.this, "Sign-in failed. Please try again.", Toast.LENGTH_SHORT).show();
                            }
                        }
                    }
                }
    ...
    private void handleGoogleSignInError(ApiException e) {
        ...
        Toast.makeText(LawyerCreateAccountActivity.this, errorMessage, Toast.LENGTH_LONG).show();
    }
                ...
                .addOnCompleteListener(this, task -> {
                    if (task.isSuccessful()) {
                        ...
                        if (user != null) {
                            ...
                            String userName = user.getDisplayName() != null ? user.getDisplayName() : "Unknown Lawyer";
                            String userEmail = user.getEmail() != null ? user.getEmail() : "No Email Provided";

                            Log.d(TAG, "User authenticated: " + userEmail);

                            // Update progress message
                            updateProgressDialog("Setting up your lawyer account...");

                            // Check if lawyer already exists in Firestore
                            firestore.collection("Users")
                                    ...
                                    .addOnCompleteListener(getTask -> {
                                        if (getTask.isSuccessful()) {
                                            ...
                                            if (!document.exists()) {
                                                Log.d(TAG, "Creating new lawyer document");

                                                // Update progress message
                                                updateProgressDialog("Creating your lawyer profile...");

                                                // Lawyer doesn't exist, create new lawyer document
                                                saveLawyerToFirestore(user, userName, userEmail, "google");
                                            } else {
                                                // Lawyer already exists, just navigate to main activity
                                                Log.d(TAG, "Lawyer already exists in Firestore");
                                            ...
                                            Log.e(TAG, "Failed to check lawyer existence", getTask.getException());
                                            hideProgressDialog();
        ...
        mAuth.createUserWithEmailAndPassword(email, password)
                .addOnCompleteListener(this, task -> {
                    if (task.isSuccessful()) {
                        ...
                        if (user != null) {
                            // Update progress message
                            updateProgressDialog("Setting up your lawyer profile...");

                            // Save lawyer data to Firestore
                            saveLawyerToFirestore(user, name, email, "email");
                        }
                    } else {
    ...
    private void saveLawyerToFirestore(FirebaseUser user, String name, String email, String signInMethod) {
        String userId = user.getUid();

        // Create lawyer data map
        Map<String, Object> lawyerData = new HashMap<>();
        lawyerData.put("uid", userId);
        lawyerData.put("name", name);
        lawyerData.put("email", email);
        lawyerData.put("role", "lawyer");
        lawyerData.put("signInMethod", signInMethod);
        lawyerData.put("createdAt", com.google.firebase.firestore.FieldValue.serverTimestamp());
        lawyerData.put("profileComplete", false);
        lawyerData.put("verified", false);

        // Save to Users/Lawyers subcollection
        firestore.collection("Users")
                .document("Lawyers")
                .collection("Lawyers")
                .document(userId)
                ...
                .addOnSuccessListener(aVoid -> {
                    Log.d(TAG, "Lawyer data saved successfully");
                    ...
                    new android.os.Handler().postDelayed(() -> {
                        hideProgressDialog();
                        Toast.makeText(LawyerCreateAccountActivity.this,
                                "Account created successfully!", Toast.LENGTH_SHORT).show();
                        navigateToNextActivity();
                    }, 500);
                })
                .addOnFailureListener(e -> {
                    Log.w(TAG, "Error saving lawyer data", e);
                    hideProgressDialog();
                    Toast.makeText(LawyerCreateAccountActivity.this,
    ...
    private void navigateToNextActivity() {
        ...
        Intent intent = new Intent(this, LawyerProfileSetupActivity.class);
